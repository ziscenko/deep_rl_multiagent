...
EPSILON_MIN = 0
LEARNING_FREQ = 1   # learning frequency  
N_UPDATES   = 1    # how much to learn

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


class MultiAgent(object):
    def __init__(self, state_size, action_size, num_agents=2, random_seed=33):
        
        self.maddpg_agents = []
        for n in range(num_agents):
            self.maddpg_agents.append(Agent(state_size, action_size, 
                                            num_agents, random_seed, n))
    ...
    def get_actions(self, states, add_noise):
        '''gets actions for each agent and combines them'''
        actions = [agent.act(states, add_noise) for agent in self.maddpg_agents]
        return np.concatenate(actions, axis=0).flatten()
    
    def reset(self):
        """Reset all agents' noise processes"""
        for agent in self.maddpg_agents: agent.reset()
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if (len(self.memory) > BATCH_SIZE) and (self.iter % LEARNING_FREQ == 0):
...
class Agent(object):
    ...
    def __init__(self, state_size, action_size, num_agents, random_seed, agent_number):
        ...
        tens = np.repeat(False, num_agents)
        tens[agent_number] = True
        self.tensor_idx = torch.BoolTensor(tens)
        
        # Actor Network (w/ Target Network)
        self.actor_local = Actor(state_size, action_size, 
                                 num_agents, random_seed).to(device)
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets = rewards[:, agent_idx] + (gamma * Q_targets_next * (1 - dones[:, agent_idx]))
        ...
        # ---------------------------- update noise ---------------------------- #
        eps_new = self.epsilon - EPSILON_DECAY
        self.epsilon = max(eps_new, EPSILON_MIN)
        self.noise.reset()
